...
class Blockchain {
    ...
    requestMessageOwnershipVerification(address) {
        return new Promise((resolve) => {
            const verificationMessage = `${address}:${new Date().getTime().toString().slice(0, -3)}:starRegistry`;
    ...
    submitStar(address, message, signature, star) {
        let self = this;
        return new Promise(async (resolve, reject) => {
            let messageSections = message.split(':');
            if (messageSections.length !== 3) {
                reject("Invalid message format");
            }
            let messageSentTime = parseInt(messageSections[1]);

            let currentTime = parseInt(new Date().getTime().toString().slice(0, -3));

            let minutesPassed = (currentTime - messageSentTime) / 60;
            if (minutesPassed >= 5) {
                reject("Over 5 minutes have passed since signing");
            }
            ...
            try {
                ...
                reject(`Signature verification failed ${error}`);
            }
    ...
    validateChain() {
        ...
        return new Promise(async (resolve, reject) => {
            self.chain.forEach((block, i) => {
                //validate previousBlockHash except for genesis block
                if (i > 0) {
